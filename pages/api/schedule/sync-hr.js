  ...
  setInterval(async () => {
    try {
      ...
      const empResult = await query(
        'SELECT name, employee_id FROM schedule_employees WHERE is_active = TRUE AND employee_type IN (\'全职用户接待岗\', \'兼职用户接待岗\') ORDER BY employee_id ASC'
      );
...
// ── 发送带附件的邮件（通过邮件 Worker + SMTP） ──
// 当前 Worker 仅支持纯文本/HTML邮件，附件功能暂不支持
// 带附件的邮件先用正文发送排班表 HTML 表格，附件功能后续扩展
async function sendMailWithAttachment(to, subject, html, attachmentBuffer, attachmentName) {
  ...
  if (!MAIL_WORKER_URL || !MAIL_API_KEY) {
    console.log('\n========== 同步人力邮件（开发模式 — 未配置 MAIL_WORKER_URL/MAIL_API_KEY）==========');
    console.log(`收件人: ${to}`);
    console.log(`主题: ${subject}`);
    console.log(`附件: ${attachmentName} (${attachmentBuffer.length} bytes)`);
    console.log('================================================\n');
    return { success: false, error: '未配置 MAIL_WORKER_URL 或 MAIL_API_KEY' };
  }

  try {
    const mailHtml = `<p>${html}</p><p>（排班表 Excel 附件暂以邮件正文形式发送，如需 Excel 文件请联系管理员。）</p>`;
    ...
    if (!response.ok) {
      ...
      let errMsg = `Mail Worker 返回 HTTP ${response.status}`;
      try {
    ...
    console.log(`[同步人力邮件已发送(Worker)] To: ${to}, Subject: ${subject}`);
    return { success: true };
  } catch (err) {
