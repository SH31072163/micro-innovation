  ...
  try {
    ...
    if (req.method === 'GET') {
      ...
      if (keyword) {
        const kw = `%${keyword}%`;
        users = await query(`
          SELECT id, user_id, username, real_name, email, phone, department, labor_relation, status, email_verified, register_date
          FROM users WHERE username ILIKE $1 OR email ILIKE $2 OR department ILIKE $3 OR real_name ILIKE $4 OR labor_relation ILIKE $5
          ORDER BY id ASC
        `, [kw, kw, kw, kw, kw]);
      } else {
        users = await query(`SELECT id, user_id, username, real_name, email, phone, department, labor_relation, status, email_verified, register_date FROM users ORDER BY id ASC`);
      }
      ...
      if (action === 'freeze') {
        ...
        const mailHtml = `
          ...
          <p>此链接24小时后失效。</p>
        `;
        const mailResult = await sendMail(targetUser.email, '【微创新实验田】请验证您的注册邮箱', mailHtml);
        if (mailResult === false) {
          return res.status(500).json({ error: '验证邮件发送失败：' + (sendMail.lastError || '未知错误') });
        }
      ...
      if (field === 'password') {
        ...
        if (targetUser.email) {
          ...
          const mailResult = await sendMail(targetUser.email, '【微创新实验田】您的密码已被修改', mailHtml);
          if (mailResult === false) {
            return res.status(500).json({ error: '密码已修改，但邮件通知发送失败' });
          }
        }
        ...
        const mailResult = await sendMail(value, '【微创新实验田】您的关联邮箱已修改', mailHtml);
        if (mailResult === false) {
          return res.status(500).json({ error: '邮箱修改成功，但邮件通知发送失败' });
        }
