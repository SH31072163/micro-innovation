...
/**
 * 判断是否为疑问句（询问 AI 能力，而非请求生成）
 * 例："你能生成图片吗？" → true（是询问，不触发生成）
 *     "帮我生成一张图片" → false（是生成请求）
 */
function isQuestion(message) {
  const msg = message.trim();
  // 以疑问语气词结尾，且包含能力问询词
  if (/[吗呢么？?]$/.test(msg) && /(能|能否|可以|会|有没有|是否|支持|是不是)/.test(msg)) return true;
  // 以疑问词开头并提到生成类动作
  if (/^(你能|您可以|能不能|能否|可不可以|平台能|网站能|系统能|你会|会不会|支持不支持|有没有)/.test(msg) && /(生成|画|制作|做|创建).{0,10}(图|图片|画像|照片|插画|视频|短片|影像|动画)/.test(msg)) return true;
  return false;
}
...
function detectImageIntent(message) {
  ...
  // 中文
  // 注意排除"流程图/架构图/思维导图/结构图"等文档类图表（非图片生成）
  if (/生成.{0,6}(图|画像|图片|照片|插画)/.test(msg) && !/生成.{0,6}(流程|架构|结构|思维导|组织|拓扑|泳道|类|时序|甘特|ER|关系|示意图|框图)/.test(msg)) return true;
  if (/画.{0,4}(一|张|幅|个|些)/.test(msg) && !/画.{0,4}(流程|架构|结构|思维导|拓扑|泳道|类图|时序|甘特|ER)/.test(msg)) return true;
  if (/^帮我画|^请画|^画一|^画个|^画张|^画幅/.test(msg) && !/(流程图|架构图|结构图|思维导图|拓扑图|泳道图|类图|时序图|甘特图)/.test(msg)) return true;
  if (/给我.{0,4}画|帮我.{0,4}画/.test(msg) && !/流程图|架构图|结构图|思维导图/.test(msg)) return true;
  if (/帮我.{0,4}(生成|做|创建).{0,4}(图|画像|图片|照片|插画)/.test(msg) && !/(流程图|架构图|结构图|思维导图|拓扑图|泳道图|类图|时序图|甘特图|ER图|示意图|框图)/.test(msg)) return true;
  // 英文
...
function detectVideoIntent(message) {
  ...
  if (/做.{0,8}(一个|一段|条|部)?(视频|短片|动画)/.test(msg)) return true;
  if (/制作.{0,4}(视频|短片|动画)/.test(msg)) return true;
