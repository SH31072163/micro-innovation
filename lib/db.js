...
/**
 * 批量执行多条查询（单次 HTTP 请求，规避 Cloudflare Workers 50 子请求限制）
 * statements: [{ text, params }] 或 SQL 数组
 * 返回各语句结果数组 [{ rows }]
 */
async function queryBatch(statements) {
  const client = getSql();
  // @neondatabase/serverless 的 transaction() 方法：
  // 单次 HTTP 请求内按顺序执行多条语句（同一条连接，等效事务）
  // 每条语句格式为 [text, params]
  const txStatements = statements.map(s => [s.text, s.params || []]);
  const results = await client.transaction(txStatements);
  return results;
}
...
async function initializeDb() {
  ...
  try {
    ...
    // 兼容已有数据库：如 users 表已存在则补充 user_id 列
    await client.query(`ALTER TABLE users ADD COLUMN IF NOT EXISTS user_id VARCHAR(20) DEFAULT ''`);
    ...
    if (parseInt(menuCount.rows[0].count) === 0) {
      ...
      await client.query("INSERT INTO menus (title, level, parent_id, sort_order, is_system) VALUES ('部门管理', 3, $1, 4, TRUE)", [smId]);
    }
    ...
    if (adminCheck.rows.length === 0) {
      ...
      await client.query(`\n        INSERT INTO users (username, password, email, phone, real_name, department, labor_relation, email_verified, status, is_admin, register_date, user_id)\n        VALUES ($1, $2, $3, $4, $5, $6, $7, TRUE, 'active', TRUE, $8, $9)\n      `, ['E31072163', hashedPassword, '', '', '超级管理员', '系统', '国脉员工', today, 'Uxj00000000']);
    }
...
module.exports = { getSql, query, queryBatch, withTransaction, initializeDb };